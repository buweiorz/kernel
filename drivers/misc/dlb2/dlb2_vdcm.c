...
struct dlb2_vdev {
	...
	/* IOMMU */
	ioasid_t pasid;
#if KERNEL_VERSION(5, 10, 135) <= LINUX_VERSION_CODE && !defined(DLB2_NEW_MDEV_IOMMUFD)
...
void dlb2_save_cmd_for_migration(struct dlb2 *dlb2, int vdev_id, u8 *data, int data_size)
{
	...
	if (DLB2_MBOX_CMD_TYPE(data) ==  DLB2_MBOX_CMD_RESET_SCHED_DOMAIN ||
		...
		//vdev->migration.mdata_size = DLB2_VF2PF_REQ_BYTES;
		if (last_reset)
			vdev->migration.mdata_size = last_reset + 6 * DLB2_VF2PF_REQ_BYTES;
		else
			last_reset = vdev->migration.mdata_size;
	}
#endif
}
...
#ifdef VFIO_REGION_TYPE_MIGRATION
	dlb2_vdcm_migration_init(vdev, DLB2_LM_MIGRATION_REGION_SIZE);
#endif
...
{
#if KERNEL_VERSION(5, 10, 135) <= LINUX_VERSION_CODE
...
static int dlb2_vdcm_dev_region_info(struct dlb2_vdev *vdev,
				     struct vfio_region_info *info,
				     struct vfio_info_cap *caps,
				     int reg_idx)
{
